...
for j in range(0,8): #j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    ...
    for i in range(0,3):  #i is for output sequence
        ...
        s2 = lis[i] #s2 indicates string 2.

        #fp1 is src file since you cant modify it you open it in read mode
        if(j<4):
            fp1 = open("In1_ALU.ckt",'r') #for input sequence 1
        else:
            fp1 = open("In2_ALU.ckt",'r') #for input sequecne 2
        #if you have any other input sequences you can add

        fp2 =open("ALUDELAY.ckt",'w') #destination file for running
        fp3 = open("DelayComp.txt",'a') #final output file
        mode1 = "RISE"
        mode2 = "RISE"
        mode3 = "FALL"
        mode4 = "FALL"
